...
=================================
Cache respons LLM berdasarkan kesamaan semantik menggunakan pgvector.
Jalankan: uvicorn p3_cache_pgvector:app --port 8003
...
LLAMA_PORT = int(os.environ.get("LLAMA_PORT", "8083"))
...
LOG_PATH = os.path.join(BASE_DIR, "llama_server_p3.log")
...
def get_db_connection():
    try:
        ...
        print(f"[P3] DB connection error: {e}")
        return None


def init_db(max_retries: int = 5, retry_delay: float = 2.0) -> bool:
    """Buat tabel semantic_cache jika belum ada, dengan retry mechanism."""
    for attempt in range(1, max_retries + 1):
        print(f"[P3] Mencoba koneksi database (percobaan {attempt}/{max_retries})...")
            ...
            try:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        ...
                        "(id SERIAL PRIMARY KEY, prompt TEXT, response TEXT, "
                        "embedding vector(384))"
                    )
                conn.commit()
                print("[P3] Database siap — tabel 'semantic_cache' sudah tersedia.")
                return True
            except Exception as e:
                print(f"[P3] Gagal membuat tabel: {e}")
                conn.rollback()
            finally:
                conn.close()
        if attempt < max_retries:
            print(f"[P3] Menunggu {retry_delay}s sebelum retry...")
            time.sleep(retry_delay)
    print("[P3] WARNING: Database tidak tersedia setelah semua percobaan, cache dinonaktifkan.")
    return False
...
async def start_llama_server() -> None:
    ...
    print(f"[P3] Menjalankan llama-server: {' '.join(cmd)}")
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        print(f"[P3] ERROR saat startup llama-server: {e}")
    yield
    stop_llama_server()
    await state["client"].aclose()


app = FastAPI(title="P3: Semantic Cache with pgvector", lifespan=lifespan)
...
async def chat(request: ChatRequest):
    ...
    if "choices" not in result:
        print(f"[P3] ERROR: Respons backend tidak terduga: {result}")
